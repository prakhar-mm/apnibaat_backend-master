import random
import os
import json
from slugify import slugify
from PIL import Image
import requests
import io
import time
# import img_gen as ig
# from img_gen import generate_image_leonardo_creative as leo_gen
# from img_gen import generate_image_dream_studio as dream_gen
# from img_gen import generate_sentence
# import spacy
# from collections import Counter
import random

from stability_sdk import client
import stability_sdk.interfaces.gooseai.generation.generation_pb2 as generation

import sys
sys.path.append('./')
import keys.ekeys as key
 
# # Load the Spacy model
# nlp = spacy.load("en_core_web_sm")

# def extract_keywords(text, max_keywords=4):
#     # Process the text with Spacy
#     doc = nlp(text)

#     # Filter out stopwords, punctuation, and pronouns
#     keywords = [
#         token
#         for token in doc
#         if not (token.is_stop or token.is_punct or token.is_space or token.lemma_ == "-PRON-")
#     ]

#     # Create a dictionary that maps lemmas to their respective Token objects
#     lemma_to_token = {token.lemma_.lower(): token for token in keywords}

#     # Count the occurrences of the keywords
#     keyword_counts = Counter([token.lemma_.lower() for token in keywords])

#     # Keep only the most common nouns, verbs, and adjectives
#     relevant_keywords = [
#         (lemma, count)
#         for lemma, count in keyword_counts.items()
#         if lemma in lemma_to_token and lemma_to_token[lemma].pos_ in {"NOUN", "VERB", "ADJ"}
#     ]

#     # Sort the keywords by count in descending order
#     sorted_keywords = sorted(relevant_keywords, key=lambda x: x[1], reverse=True)

#     # Return the top 'max_keywords' keywords
#     return [keyword for keyword, count in sorted_keywords[:max_keywords]]

# def summarize_text(text):
#     # Process the text with Spacy
#     doc = nlp(text)
    
#     # Extract keywords from the text
#     keywords = extract_keywords(text)
    
#     # Tokenize the text into sentences
#     sentences = list(doc.sents)
    
#     # Score each sentence based on the number of relevant keywords it contains
#     sentence_scores = [
#         (sentence, sum(1 for token in sentence if token.lemma_.lower() in keywords))
#         for sentence in sentences
#     ]
    
#     # Find the sentence with the highest score
#     best_sentence = max(sentence_scores, key=lambda x: x[1])[0]
    
#     return best_sentence.text



# # Image styling module
# styles_dict = {
#     # 1: "16-Bit",
#     # 2: "1800s",
#     # 3: "1980s",
#     # 4: "4-Bit",
#     # 5: "8-Bit",
#     # 6: "Amber",
#     # 7: "Anatomical Drawing",
#     # 8: "Ancient",
#     # 9: "Anime",
#     # 10: "Antimatter",
#     10: "3D Model",
#     11: "Arabic",
#     12: "Black Hole",
#     13: "Realistic",
#     14: "Oil painting",
#     15: "Concept art"
    
# }

# lighting_dict = {
#     1: "Accent Lighting",
#     2: "Backlight",
#     3: "Blacklight",
#     4: "Blindinglight",
#     5: "Candlelight",
#     6: "Concert Light",
#     7: "Crepuscular Rays",
#     8: "Direct Sunlight",
#     9: "Dusk",
#     10: "Edison Bulb",
#     11: "Electric Arc",
#     12: "Fire"
# }

# camera_dict = {
#     1: "360 Panorama",
#     2: "DSLR",
#     3: "Electron Microscope",
#     4: "Macro lens",
#     5: "Magnification",
#     6: "Microscopy",
#     7: "Miniature Faking",
#     8: "Panorama",
#     9: "Pinehole lens",
#     10: "Satellite Imagery",
#     11: "Super resolution microscopy",
#     12: "Telephoto lens"
# }

# artists_dict = {
#     1: "John Singer Sargent",
#     2: "Edgar Degas",
#     3: "Paul Cézanne",
#     4: "Jan van Eyck",
#     5: "Leonardo DaVinci",
#     6: "Vincent Van Gogh",
#     7: "Johannes Vermeer",
#     8: "Rembrandt",
#     9: "Albrecht Dürer",
#     10: "Leonardo da Vinci",
#     11: "Michelangelo",
#     12: "Jean-Auguste-Dominique Ingres",
#     13: "Thomas Moran",
#     14: "Claude Monet",
#     15: "Alfred Bierstadt",
#     16: "Frederic Edwin Church",
#     17: "Alphonse Mucha",
#     18: "Andy Warhol",
#     19: "Art by Yoko Ono",
#     20: "Banksy",
#     21: "By Francisco De Goya",
#     22: "Caravaggio",
#     23: "David Hockney",
#     24: "Diego Rivera",
#     25: "Eugene Delacroix",
#     26: "Francis Bacon",
#     27: "Frida Kahlo"
# }

# color_dict = {
#     1: "Amber",
#     2: "Baby Blue Color",
#     3: "Baby Pink Color",
#     4: "Beige",
#     5: "Blue",
#     6: "Brown color",
#     7: "CYMK",
#     8: "Citrus",
#     9: "Coquelicot color",
#     10: "Cyan",
#     11: "Gold color",
#     12: "Gray"
# }

# material_dict = {
#     1: "Aluminium",
#     2: "Brick",
#     3: "Bronze",
#     4: "Carbon Fibre",
#     5: "Cardboard",
#     6: "Cellulose",
#     7: "Ceramic",
#     8: "Cotton",
#     9: "Fabric",
#     10: "Fiber Optic",
#     11: "Foil",
#     12: "Gasoline"
# }


# def pick_random_style(styles):
#     random_key = random.choice(list(styles.keys()))
#     random_style = styles[random_key]
#     return f"{random_style}"

# def is_text_about_people(text):
#     doc = nlp(text)
#     for ent in doc.ents:
#         if ent.label_ == "PERSON":
#             return True
#     return False

# def generate_sentence(title,text):
#     random_style = pick_random_style(styles_dict)
#     random_lighting = pick_random_style(lighting_dict)
#     random_camara = pick_random_style(camera_dict)
#     random_artist = pick_random_style(artists_dict)
#     random_color = pick_random_style(color_dict)
#     random_material = pick_random_style(material_dict)
#     keywords_array = extract_keywords(text)
#     keywords=", ".join(keywords_array)
#     best_sentence = summarize_text(text)
#     ArticleAboutPeople=is_text_about_people(text)

#     Prompt1 = f"{title}. {best_sentence}. Emphasize {keywords}, realistic and {random_style}, Lighting:{random_lighting}, Camara:{random_camara}, Material:{random_material}"
#     Prompt2 = f"{title}. {best_sentence}. Emphasize {keywords}, If using faces use Indian/North Indian/East Indian faces,realistic,avoid using hands,skintone can vary from white to brown, with realistic background, background can be blur, Style: {random_style}, Lighting:{random_lighting}, Camara:{random_camara},  Material:{random_material}"
#     ArticleAboutPeople=is_text_about_people(Prompt1)
#     print(f"ArticleAboutPeople: {ArticleAboutPeople}")
#     if ArticleAboutPeople == True:
#         return Prompt2
#     return Prompt1


os.environ['STABILITY_HOST'] = key.stability_host
os.environ['STABILITY_KEY'] = key.stability_key

stability_api = client.StabilityInference(
    key=os.environ['STABILITY_KEY'],
    verbose=True,
    engine="stable-diffusion-xl-beta-v2-2-2",
)

def dream_gen(prompt, output_name, output_folder=None, engine_id="stable-diffusion-xl-beta-v2-2-2", width=512, height=512):
    answers = stability_api.generate(
        prompt=prompt,
        steps=50,
        cfg_scale=7.0,
        width=width,
        height=height,
        samples=1,
        sampler=generation.SAMPLER_K_DPMPP_2M
    )
    
    for answer in answers:
        result_image_data = answer.artifacts[0].binary

    img = Image.open(io.BytesIO(result_image_data))
    # img.show()
    
    # upscale the image to double its size
    upscaled_img = img.resize((img.size[0]*2, img.size[1]*2))
    # upscaled_img.show()

    # Save the upscaled image to a file
    if output_folder is not None:
        os.makedirs(output_folder, exist_ok=True)
        output_path = os.path.join(output_folder, output_name)
    else:
        output_path = output_name
    upscaled_img.save(output_path)


os.environ["LEONARDO_API_KEY"] = key.leonardo_key
apikey = os.environ["LEONARDO_API_KEY"]

def leo_gen(prompt, output_name, output_folder=None):
    url = "https://cloud.leonardo.ai/api/rest/v1/generations"

    payload = {
        "prompt": prompt,
        "modelId": "6bef9f1b-29cb-40c7-b9df-32b51c1f67d3",
        #"modelId": "f3296a34-9aef-4370-ad18-88daf26862c3",
        
        "width": 768,
        "height": 768,
        "sd_version": "v2",
        "num_images": 1,
        "num_inference_steps": 30,
        "guidance_scale": 7,
        "presetStyle": "LEONARDO",
        # "tiling": True,
        # "public": True,
        "promptMagic": True
    }
    headers = {
        "accept": "application/json",
        "content-type": "application/json",
        "authorization": f"Bearer {apikey}"
    }

    # First API call: POST request to start image generation
    response = requests.post(url, json=payload, headers=headers)
    response_json = json.loads(response.text)

    generation_id = response_json["sdGenerationJob"]["generationId"]

    # Build the URL with the base URL and the generationId
    base_url = "https://cloud.leonardo.ai/api/rest/v1/generations/"
    url_with_generation_id = f"{base_url}{generation_id}"

    # Poll the API until the status is "COMPLETE"
    while True:
        response = requests.get(url_with_generation_id, headers=headers)
        response_json = json.loads(response.text)

        status = response_json["generations_by_pk"]["status"]
        
        if status == "COMPLETE":
            break
        else:
            time.sleep(5)  # Wait for 5 seconds before checking again

    # Second API call: GET request to get the generated image URL
    image_url = response_json["generations_by_pk"]["generated_images"][0]["url"]

    # Make a GET request to download the image
    response_image = requests.get(image_url)
    img = Image.open(io.BytesIO(response_image.content))

    # Save the image to a file
    if output_folder is not None:
        os.makedirs(output_folder, exist_ok=True)
        output_path = os.path.join(output_folder, output_name)
    else:
        output_path = output_name
    img.save(output_path)


input_path = 'outputs/authors/cleaned_author_category_scores.json'
images_path = 'outputs/authors/images'
if not os.path.exists(images_path):
    os.mkdir(images_path)

def generate_random_image(prompt, output_name, output_folder=images_path):
    functions = [
            # leo_gen,
            dream_gen
        ]

    # Randomly select a function
    selected_function = random.choice(functions)

    try:
        # Call the selected function
        selected_function(prompt, output_name, output_folder)

        # Return the name of the selected function
        return selected_function.__name__
    except Exception as e:
        print(f"Failed with {selected_function.__name__}: {e}")

        # Choose the other function as a fallback
        fallback_function = [func for func in functions if func != selected_function][0]
        
        print(f"Trying fallback function {fallback_function.__name__}")
        try:
            # Call the fallback function
            fallback_function(prompt, output_name, output_folder)

            # Return the name of the fallback function
            return fallback_function.__name__
        except Exception as e:
            print(f"Failed with fallback function {fallback_function.__name__}: {e}")
            return None
        
with open(input_path) as f:
    data = json.load(f)

new_data = {}
# function calling and saving images which we like and discarding others
for name,tags in data.items():
    # print(name)
    # print(tags['bio'])

    slug = slugify(name)
    image_filename = f"{slug}.png"
    image_path = os.path.join(images_path, image_filename)

    if os.path.exists(image_path):
        print(f"Image for author {name} already exists. Skipping.")
        image_url = os.path.join(images_path, image_filename)
        tags.update({"Image_url": image_url, "Slug": slug})
        new_data[name] = tags
        continue

    bio = tags['bio']
    add_str = "Human like, author, journalist, realistic blurred background, A realistic professional portrait image of an author who's bio is"
    while(True):
        try:
            # generated_sentence = generate_sentence("A realistic human like portrait image of an author who's bio is", bio)
            generated_sentence =  bio
            print(name)
            generated_sentence = add_str+ ',' +generated_sentence
            print(generated_sentence)

            image_source = generate_random_image(generated_sentence, image_filename, images_path)
            image_url = os.path.join(images_path, image_filename)
            image = Image.open(image_url)
            image.show()
            
            user_input = input("Do you like the image? (y/n/e): ")
            if user_input.lower() == 'y':
                # Move the image to the desired location for permanent storage
                print("Image saved successfully.")
                tags.update({"Image_url": image_url, "Slug": slug, "Image_source": image_source})
                new_data[name] = tags
                break
            else:
                # Delete the temporary image file
                add_str = input("wanna add some extra prompt for img gen? ") + add_str
                os.remove(image_url)
                print("Image discarded. retrying ...")
                if user_input.lower() == 'e':
                    break
            

        except Exception as e:
            print(f"Failed to generate image of author {name}: {e}")

    user_input = input("Do you wanna exit the loop (y/n): ")
    if user_input.lower() == 'y':
        break

with open('./outputs/authors/author_imgs_path.json', 'w') as f:
    json.dump(new_data, f, indent=4)
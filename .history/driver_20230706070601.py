    ...
    for script in scripts:
        ...
        if script == './data_gen_load/img_gen.py':
            ...
            with open('outputs/json_with_img_path/article_data_imageUrl_slug.json') as f:
                data = json.load(f)
            for i, (topic, article) in enumerate(data.items()):
                ...
                while user_choice is None:
                    proceed = st.button(f"Yes {i}", key=f"proceed_{script}_{i}", help="Click 'Yes' to continue")
                    abort = st.button(f"No {i}", key=f"abort_{script}_{i}", help="Click 'No' to abort the execution")
                ...
                if user_choice == "proceed":
                    st.write("User selected 'Yes'. Proceeding...")
                ...
                if user_choice == "abort":
                    ...
                    st.error("User selected 'No'. Execution aborted.")

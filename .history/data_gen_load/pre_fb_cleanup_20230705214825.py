...
def get_top_author(category):
    # Get all the author names from the category scores dictionary
    author_names = list(category_scores.keys())

    # Randomly select an author name from the list
    random_author = random.choice(author_names)

    # Get the bio of the selected author
    author_bio = category_scores[random_author].get('bio', 'Default author bio')

    return {"name": random_author, "bio": author_bio}
...
for article_key, item in data.items():
    ...
    # Clean up the title    
    title = extract_start_string(item['title'])
    ...
    item['cate'] = category
    item['cate_bg'] = color_map.get(item['cate'], 'bg-color-teal')
...
for topic_key, topic in input_data.items():
    tags = topic["keywords"].split(",")  # Split the string by comma to create a list
    products = topic["Products"].split(",")  # Split the string by comma to create a list
    output_data = {
        "title": topic["title"],
        "summary": topic["excerpt"],
        "apnibaat_post_unique_key": topic["apnibaat_post_unique_key"],
        "excerpt": topic["excerpt"],
        "slug": topic["slug"],
        "cate": topic["cate"],
        "cate_bg": topic["cate_bg"],
        "content": topic["content"],
        "post_views": topic["post_views"],
        "post_share": topic["post_share"],
        "author_name": topic["author_name"],
        "author_desg": topic["author_desg"],
        "author_bio": topic["author_bio"],
        "postFormat": topic["postFormat"],
        "trending": topic["trending"],
        "date": topic["date"],
        "quoteText": topic["quoteText"],
        "featureImg": topic["featureImg"],
        "author_social": topic["author_social"],
        "author_img": topic["author_img"],
        "tags": tags,  # Assign the list of tags
        "products": products
    }
...
output_dir = "./outputs/final_cleaned_json/"
os.makedirs(output_dir, exist_ok=True)
filename = f"output_{now}.json"
output_path = os.path.join(output_dir, filename)

# Delete any existing files in the final_jsons folder

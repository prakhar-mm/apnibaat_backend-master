    ...
    for i, script in enumerate(scripts):
        ...
        if script == './data_gen_load/img_gen.py':
            ...
            for topic, article in data.items():
                ...
                if i == 1:  # Only for img_gen.py script
                    proceed = st.button("Proceed", key=f"proceed_{script}", help="Click to proceed with the next script")
                    abort = st.button("Abort", key=f"abort_{script}", help="Click to abort the subsequent script execution")
                    ...
                    proceed_button_clicked = True  # Continue for other scripts

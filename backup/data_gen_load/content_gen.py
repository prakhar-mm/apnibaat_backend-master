...
for key in input_keys:
    ...
    response = chat_gpt(f"Give me {num_of_ideas} article ideas for indian audiences or just redefine my content idea-", content_idea)
    article_ideas = response.strip().split("\n")
    content_data = {}
    content_data[category] = {}
    for i, article_idea in enumerate(article_ideas):
        response = chat_gpt("In not more than thousand words and Keeping in mind that the article is for indian audience Write an article on:", article_idea)
